...
class CrudIncident:
    ...
    async def create_incident(self, *, incident_in: IncidentCreate) -> Incident:
        ...
        db_incident.impacts = Impacts(**incident_in.impacts.model_dump())
        db_incident.shallow_rca = ShallowRCA(
            **incident_in.shallow_rca.model_dump())
        db_incident.affected_items = [AffectedItem(
            **item.model_dump()) for item in incident_in.affected_items]
        ...
        # We flush to send changes to the DB and get IDs, but the service layer handles the commit.
        await self.db.flush()
        await self.db.refresh(db_incident)
        return db_incident

    async def search_incidents(
        self, *, statuses: Optional[List[IncidentStatusEnum]] = None, severities: Optional[List[SeverityLevelEnum]] = None,
        commander_id: Optional[UUID] = None, start_date: Optional[str] = None, end_date: Optional[str] = None,
        ...
        if start_date:
            conditions.append(IncidentProfile.datetime_detected_utc >=
                              datetime.fromisoformat(start_date))
        if end_date:
            conditions.append(
                IncidentProfile.datetime_detected_utc <= datetime.fromisoformat(end_date))
        if conditions:
            statement = statement.where(and_(*conditions))
        statement = statement.options(
            selectinload(Incident.profile),
            selectinload(Incident.affected_items)
        ).order_by(Incident.created_at.desc()).offset(skip).limit(limit)
        result = await self.db.exec(statement)
    ...
    async def count_incidents(
        ...
        statement = select(func.count(Incident.id)).join(IncidentProfile)
        conditions = []
        if statuses:
            conditions.append(IncidentProfile.status.in_(statuses))
        if severities:
    ...
    async def update_incident_profile(self, *, db_incident: Incident, update_data: Dict[str, Any]) -> Incident:
    ...
    async def update_incident_impacts(self, *, db_incident: Incident, impacts_data: Dict[str, Any]) -> Incident:
    ...
    async def update_shallow_rca(self, *, db_incident: Incident, rca_data: Dict[str, Any]) -> Incident:
    ...
    async def update_resolution(self, *, db_incident: Incident, resolution_data: ResolutionMitigationCreate) -> Incident:
        if db_incident.resolution_mitigation:
            for field, value in resolution_data.model_dump(exclude_unset=True).items():
                setattr(db_incident.resolution_mitigation, field, value)
        else:
    ...
    async def add_timeline_event(self, *, incident: Incident, new_event: TimelineEvent) -> Incident:
