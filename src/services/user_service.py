...
# Import CRUD operations, models, schemas, security utils, and custom exceptions using absolute paths from 'src'
...
from src.api.v1.schemas.auth_schemas import PasswordResetRequest, PasswordResetConfirm
from src.core.security import get_password_hash, verify_password, create_access_token, decode_token
# Corrected import path for exceptions: import from the package __init__
...
class UserService:
    ...
    async def register_user(self, *, user_in: UserCreate) -> User:
        ...
        """
        existing_user_by_username = await self.crud_user.get_user_by_username(username=user_in.username)
        if existing_user_by_username:
            raise DuplicateResourceException(
                detail=f"Username '{user_in.username}' is already registered.")

        existing_user_by_email = await self.crud_user.get_user_by_email(email=user_in.email)
        ...
        created_user = await self.crud_user.db_create_user(user_in=user_create_internal_data)
    ...
    async def authenticate_user(self, *, username: str, password: str, client_ip: str | None = None) -> User:
        ...
        """
        user = await self.crud_user.get_user_by_username(username=username)
        if not user:
    ...
    async def update_user_profile(self, *, current_user: User, user_in: UserUpdate) -> User:
        ...
        if "email" in update_data and update_data["email"] != current_user.email:
            existing_user = await self.crud_user.get_user_by_email(email=update_data["email"])
        ...
        if "username" in update_data and update_data["username"] != current_user.username:
            existing_user = await self.crud_user.get_user_by_username(username=update_data["username"])
            if existing_user and existing_user.id != current_user.id:
                raise DuplicateResourceException(
    ...
    async def request_password_reset(self, *, email_in: PasswordResetRequest) -> str:
        """
        ...
        reset_token_payload = {"sub": str(
            user.id), "type": "password_reset", "email": user.email}
        ...
        update_data = {
            ...
            "reset_token_expires": datetime.utcnow() + timedelta(minutes=password_reset_token_expire_minutes)  # Use utcnow()
        }
        await self.crud_user.update_user(db_user_to_update=user, user_in_update_data=update_data)

        print(
            f"TODO: Send password reset email to {user.email} with token: {reset_token}")

        return "If your email is registered and active, you will receive a password reset link shortly."
    ...
    async def confirm_password_reset(self, *, token_in: str, new_password_in: PasswordResetConfirm) -> User:
        ...
        if not payload or payload.get("type") != "password_reset":
            raise InvalidInputException(
                detail="Invalid or expired password reset token (type or payload error).")
        ...
        if datetime.utcnow() > user.reset_token_expires:  # Use utcnow()
            await self.crud_user.update_user(db_user_to_update=user, user_in_update_data={"reset_token": None, "reset_token_expires": None})
            raise InvalidInputException(
        ...
        updated_user = await self.crud_user.update_user(db_user_to_update=user, user_in_update_data=update_data)
        return updated_user

    async def request_email_verification_token(self, *, current_user: User) -> str:
        """
        ...
        email_verify_token_expire_minutes = getattr(
            settings, 'EMAIL_VERIFY_TOKEN_EXPIRE_MINUTES', 1440)

        verify_token_payload = {"sub": str(
            current_user.id), "type": "email_verification", "email": current_user.email}
        email_verification_token = create_access_token(
            subject=verify_token_payload,
            expires_delta=timedelta(minutes=email_verify_token_expire_minutes)
        )

        update_data = {
            "email_verification_token": get_password_hash(email_verification_token)
        }
        # You might want to add email_verification_token_expires_at to User model if needed
        await self.crud_user.update_user(db_user_to_update=current_user, user_in_update_data=update_data)
    ...
    async def confirm_email_verification(self, *, token_in: str) -> User:
        ...
        if not payload or payload.get("type") != "email_verification":
            raise InvalidInputException(
                detail="Invalid or expired email verification token (type or payload error).")
        ...
        if not user.email_verification_token:
            raise InvalidInputException(
                detail="No pending email verification for this user or token already used/invalid.")

        if not verify_password(token_in, user.email_verification_token):
            raise InvalidInputException(
                detail="Invalid email verification token.")

        if token_email and user.email.lower() != token_email.lower():  # Check if token_email exists before lower()
            raise InvalidInputException(
        ...
        update_data = {
            ...
            "email_verified_at": datetime.utcnow(),  # Use utcnow()
            "is_active": True
        }
        updated_user = await self.crud_user.update_user(db_user_to_update=user, user_in_update_data=update_data)
        return updated_user
    ...
    async def soft_delete_user(self, *, user_to_delete_id: UUID, performing_user: User) -> User:
        ...
        update_data = {
            "is_deleted": True,
            "deleted_at": datetime.utcnow(),  # Use utcnow()
            "is_active": False
        }
        deleted_user = await self.crud_user.update_user(
            db_user_to_update=user_to_delete, user_in_update_data=update_data
        )
        return deleted_user
