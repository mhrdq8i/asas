...
class PostmortemService:
    ...
    async def _get_postmortem_or_fail(
        ...
        postmortem = await \
            self.crud_postmortem.get_postmortem_by_id(
                postmortem_id=postmortem_id
            )

        if not postmortem:
            raise PostMortemNotFoundException(
                identifier=postmortem_id
            )
    ...
    async def _check_permission(
        ...
        *,
        ...
        for post-mortem management.
        """

        is_commander = \
            incident.profile.commander_id == user.id

        is_superuser = user.is_superuser

        if not (
            is_commander or is_superuser
        ):
            raise InsufficientPermissionsException(
                "You do not have permission "
                "to manage this post-mortem."
            )
    ...
    async def create_postmortem(
        ...
        *,
        ...
        """
        Creates a new draft post-mortem for a
        ...
        if incident.profile.status != \
            ...
            raise IncidentNotResolvedException(
                incident_id=incident_id
            )
        ...
        if existing_postmortem:
            raise PostMortemAlreadyExistsException(
                incident_id=incident_id
            )
        ...
        db_postmortem = await \
            self.crud_postmortem.create_postmortem(
                postmortem=new_postmortem
            )

        await self.db_session.commit()

        # Eagerly load relationships after commit
        # to prevent MissingGreenlet error.
        refreshed_pm = await \
    ...
    async def get_all_postmortems(
        ...
        result = await \
            self.crud_postmortem.get_all_postmortems(
                skip=skip,
                limit=limit
            )
    ...
    async def update_postmortem(
        ...
        *,
        ...
        if update_dict.get(
            ...
            PostMortemStatusEnum.COMPLETED
            and
            db_postmortem.status
            !=
            PostMortemStatusEnum.COMPLETED
        ):
            update_dict[
        ...
        updated_pm = await \
            self.crud_postmortem.update_postmortem(
    ...
    async def delete_postmortem(
        ...
        *,
        ...
        # We need the associated
        # incident to check permissions
        incident = await \
            self._get_incident_or_fail(
                db_postmortem.incident_id
            )
