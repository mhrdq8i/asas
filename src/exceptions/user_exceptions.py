...
class UserNotFoundException(ResourceNotFoundException):
    ...
    def __init__(self, identifier: Any = None, detail: str | None = None):
        super().__init__(resource_name="User", identifier=identifier)
        if detail:
            # If a custom detail is provided, override the default message.
            self.detail = detail


class UserAlreadyDeletedException(InvalidOperationException):
    """
    Raised when an operation is attempted on an already soft-deleted user.
    """

    def __init__(self, detail: str = "User is already soft-deleted."):
        super().__init__(detail=detail)


class CannotDeleteActiveCommanderException(InvalidOperationException):
    """
    Raised when attempting to delete a user who is an active incident commander.
    """

    def __init__(self, detail: str = "User is an active incident commander and cannot be deleted."):
        super().__init__(detail=detail)
